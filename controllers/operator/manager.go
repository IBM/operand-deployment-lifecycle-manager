...
func (m *ODLMOperator) GetDeployment(ctx context.Context, name, operatorNs, servicesNs, packageName string) (*appsv1.Deployment, error) {
	...
	if name != "" {
		err := m.Client.Get(ctx, types.NamespacedName{Name: name, Namespace: operatorNs}, deploy)
		if err == nil {
			return deploy, nil
		}
		// klog.Warningf("Deployment %s not found in namespace %s, checking packageName %s", name, operatorNs, packageName)
	}

	if name != "" {
		err := m.Client.Get(ctx, types.NamespacedName{Name: packageName, Namespace: operatorNs}, deploy)
		if err == nil {
			return deploy, nil
		}
		// klog.Warningf("Deployment %s not found in namespace %s, checking packageName %s", name, operatorNs, packageName)
	...
	// 	err := m.Client.List(ctx, deployList, client.InNamespace(operatorNs))
	// 	if err != nil {
	// 		klog.Errorf("Failed to list Deployments in namespace %s: %v", operatorNs, err)
	// 		return nil, err
	// 	}
	...
	klog.Errorf("No Deployment found with name=%s or packageName=%s in namespace=%s", name, packageName, operatorNs)
	return nil, fmt.Errorf("no Deployment found with name=%s or packageName=%s in namespace=%s", name, packageName, operatorNs)

	// tenantScope := make(map[string]struct{})
	// for _, ns := range []string{operatorNs, servicesNs} {
	// 	tenantScope[ns] = struct{}{}
	// 	klog.Infof("22-22 %v", tenantScope)
	// }
