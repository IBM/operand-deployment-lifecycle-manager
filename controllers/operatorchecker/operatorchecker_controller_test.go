...
var _ = Describe("OperatorChecker controller", func() {
	const (
		name              = "common-service"
	...
	var (
		...
		operatorCheckerKey    types.NamespacedName
	)
	...
	BeforeEach(func() {
		...
		operatorCheckerKey = types.NamespacedName{Name: name, Namespace: namespaceName}
	...
	Context("Initializing OperatorChecker Status", func() {

		It("Should status of OperatorChecker be waiting for CatalogSource", func() {

			By("Checking status of the OperatorChecker")
			Eventually(func() operatorv1alpha1.OperatorCheckerPhase {
				operatorCheckerInstance := &operatorv1alpha1.OperatorChecker{}
				Expect(k8sClient.Get(ctx, operatorCheckerKey, operatorCheckerInstance)).Should(Succeed())
				return operatorCheckerInstance.Status.Phase
			}, timeout, interval).Should(Equal(operatorv1alpha1.OperatorCheckerReady))
			...
			Eventually(func() operatorv1alpha1.OperatorCheckerPhase {
				...
				Expect(k8sClient.Get(ctx, operatorCheckerKey, OperatorCheckerInstance)).Should(Succeed())
				return OperatorCheckerInstance.Status.Phase
			}, timeout, interval).Should(Equal(operatorv1alpha1.OperatorCheckerRunning))
