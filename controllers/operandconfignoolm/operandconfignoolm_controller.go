...
func (r *Reconciler) updateStatus(ctx context.Context, instance *operatorv1alpha1.OperandConfig) error {
	...
	for _, op := range registryInstance.Spec.Operators {
		...
		klog.V(1).Info("Looking for deployment for the operator: ", op.Name)
		var deployment *appsv1.Deployment
		deploymentList, err := r.GetDeploymentListFromPackage(ctx, op.PackageName, op.Namespace)
		deployment = deploymentList[0]

		if deployment == nil {
			klog.Warningf("Deployment for the Operator %s/%s doesn't exist, retry...", namespace, op.Name)
			continue
		}
		...
		if almExamples == "" {
			klog.Warningf("Not found alm-examples in the Deployment %s/%s", &deployment.Namespace, &deployment.Name)
			continue
		}
		...
		if err != nil {
			return errors.Wrapf(err, "failed to convert alm-examples in the Deployment %s/%s to slice", &deployment.Namespace, &deployment.Name)
		}
